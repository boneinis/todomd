...
Build selection is naturally exclusive (the card flips to `Build` under the lock). Triage and planning, though, run a long agent step *before* the card's status changes — so a budget dispatcher also stamps a short-lived **lease** (`lease: "<epoch> <worker>"` in the card's frontmatter, claimed under the lock) before it starts, and other dispatchers skip any card with a fresh lease. The lease is cleared when the result is recorded and auto-expires after 30 minutes if the dispatcher crashes. This stops two dispatchers from planning or triaging the same card and throwing one result away.
