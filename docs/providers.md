...
|---|---|---|
| `permissions.allow` / `permissions.deny` | which shell commands the agent may run at all | the CLI's own `settings.json` |
...
**Grant the exploration set as well as git.** Rules match a command by prefix at
a word boundary (`git` matches `git add`, but not `github`):
...
```jsonc
{
  "permissions": {
    "allow": [
      ...
      "command(git rev-parse)", "command(git add)", "command(git commit)"
    ],
    "deny": [
      "command(npm)", "command(pnpm)", "command(yarn)", "command(pip)",
...
- The CLI's `--add-dir` does not fix this. Granting the main repository makes the
  metadata *readable* (`git status` works) but writes are still refused
  (`Unable to create .git/index.lock: Operation not permitted`), and it also
...
bounds them is the `permissions.allow` / `permissions.deny` pair above — so that
