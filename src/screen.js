...
const AUTO_REPLY_SUBJECT_RE = /^(?:\[\s*(?:(?:auto[- ]?|automatic |automated )(?:reply|response)|auto[- ]?responder)\s*\](?:\s|$)|(?:(?:auto[- ]?|automatic |automated )(?:reply|response)|auto[- ]?responder)(?:\s*[:–—-]|\s*$)|out[- ]of[- ](?:the[- ])?office(?:\s+auto[- ]?(?:reply|response))?\s*(?:[:–—-]|$))/i;
const AUTO_REPLY_BODY_RE = /(?:^|[.!?]\s+|\n)\s*(?:this is (?:an? )?(?:(?:auto[- ]?|automatic |automated )(?:reply|response)\b(?=\s*(?:[.!,:;]|$|to\b|confirming\b|because\b))|auto[- ]?responder\b)|i(?: am|['’]m| will be|['’]ll be) (?:currently )?(?:out[- ]of[- ](?:the[- ])?office\b(?!\s+(?:settings?|page|form|feature|calendar|request|policy)\b)|on vacation\b(?!\s+(?:settings?|page|form|feature|calendar|request|policy)\b)|on (?:(?:[a-z-]+\s+){0,2}leave|holiday|pto)\b(?=\s*(?:[.!?](?:\s|$)|$|[,;:]\s*(?:returning\b|until\b|through\b|back\b|and (?:i |will)\b)|until\b|through\b|today\b|tomorrow\b|this week\b|next week\b|and (?:i |will)\b|returning\b))|away from (?:my |the )?(?:office|email|desk)|away on (?:(?:[a-z-]+\s+){0,2}leave|holiday|pto)\b(?=\s*(?:[.!?](?:\s|$)|$|[,;:]\s*(?:returning\b|until\b|through\b|back\b|and (?:i |will)\b)|until\b|through\b|today\b|tomorrow\b|this week\b|next week\b|and (?:i |will)\b|returning\b))|away\b(?=\s+(?:until|through|today|tomorrow|this week|next week|on leave)\b))|currently\s+(?:out[- ]of[- ](?:the[- ])?office\b(?!\s+(?:settings?|page|form|feature|calendar|request|policy)\b)|on (?:(?:[a-z-]+\s+){0,2}leave|holiday|pto)\b(?=\s*(?:[.!?](?:\s|$)|$|[,;:]\s*(?:returning\b|until\b|through\b|back\b|and (?:i |will)\b)|until\b|through\b|today\b|tomorrow\b|this week\b|next week\b|and (?:i |will)\b|returning\b)))|ooo\b(?=\s+(?:until|through|today|tomorrow|this week|next week|on leave)\b))/i;
...
export function screenEmail(parsed) {
  ...
  const htmlText = stripHtml(html).trim();
  const bodyText = [text, htmlText].filter(Boolean).join('\n');
  ...
  const unclear = [];
  const meaningfulBody = text || htmlText;
  if (!meaningfulBody) unclear.push('empty-body');
  else if (meaningfulBody.length < MIN_BODY_LEN) unclear.push('short-body');
