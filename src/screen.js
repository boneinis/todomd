...
const AUTO_REPLY_BODY_RE = /(?:^|[.!?]\s+|\n)\s*(?:this is (?:an? )?(?:(?:auto[- ]?|automatic |automated )(?:reply|response)\b(?=\s*(?:[.!,:;]|$|to\b|confirming\b|because\b))|auto[- ]?responder\b)|i(?: am|['’]m| will be|['’]ll be) (?:currently )?(?:out[- ]of[- ](?:the[- ])?office\b(?!\s+(?:settings?|page|form|feature|calendar|request|policy)\b)|on vacation\b(?!\s+(?:settings?|page|form|feature|calendar|request|policy)\b)|on (?:(?:[a-z-]+\s+){0,2}leave|holiday|pto)\b(?!\s+(?:settings?|page|form|feature|calendar|request|policy)\b)|away from (?:my |the )?(?:office|email|desk)|away on (?:(?:[a-z-]+\s+){0,2}leave|holiday|pto)\b(?!\s+(?:settings?|page|form|feature|calendar|request|policy)\b)|away\b(?=\s+(?:until|through|today|tomorrow|this week|next week|on leave)\b))|currently\s+(?:out[- ]of[- ](?:the[- ])?office|on (?:(?:[a-z-]+\s+){0,2}leave|holiday|pto))\b(?!\s+(?:settings?|page|form|feature|calendar|request|policy)\b)|ooo\b(?=\s+(?:until|through|today|tomorrow|this week|next week|on leave)\b))/i;
