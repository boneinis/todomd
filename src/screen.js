...
const AUTO_REPLY_BODY_RE = /(?:^|[.!?]\s+|\n)\s*(?:this is (?:an? )?(?:(?:auto[- ]?|automatic |automated )(?:reply|response)\b(?=\s*(?:[.!,:;]|$|to\b|confirming\b|because\b))|auto[- ]?responder\b)|i(?: am|'m| will be) (?:currently )?(?:out[- ]of[- ](?:the[- ])?office|on vacation|on annual leave|away from (?:my |the )?(?:office|email|desk)|away\b(?=\s+(?:until|through|today|tomorrow|this week|next week|on leave)\b))|ooo\b(?=\s+(?:until|through|today|tomorrow|this week|next week|on leave)\b))/i;
