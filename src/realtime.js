...
const INSTRUCTIONS = [
  ...
  'card could match what was said, ask a short clarifying question instead of',
  'guessing.',
  'propose_board_action accepts only these action values, and no others:',
  'retriage (send a card back to Review), approve (move a card to Queue and',
  'start, or dispatcher-queue, its build), retry_planned (send a Needs Human',
  'card back to Planned), resume_build (continue a stalled build in its',
  'EXISTING preserved worktree — no prior work is lost), retry_verification',
  '(re-run only verification in the preserved worktree), restart_build',
  '(DISCARD the preserved worktree and start the build completely over — only',
  'propose this when the user clearly wants to discard prior work, never as',
  'another way to say resume_build), cancel (stop a running or queued run),',
  'archive, and unarchive.',
  'When propose_board_action succeeds, speak its readback text exactly as',
  'given, then exactly its confirmation instruction — the required spoken',
  'phrase, or that visible approval in the app is required — with no',
  'paraphrasing. Never announce that an action succeeded, failed, or was',
  'cancelled yourself; TODOMD reports that outcome separately once it is known.',
].join(' ');
